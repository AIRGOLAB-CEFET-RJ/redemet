...
def start_data(api_key, station, start_date, end_date, output_file, log_file):
    ...
    while current_date <= end_date:
        try:
            ...
            if "429" in str(e):
                ...
                print(f"Erro 429: Muitas solicitações. Aguardando 60 segundos.")
                log_entry = {
                    "Data": current_date,
    ...
    for retry_date in retry_data:
        try:
            url = requests.get(f"https://api-redemet.decea.mil.br/aerodromos/info?api_key={api_key}&localidade={station}&datahora={retry_date}")
            url.raise_for_status()
            clima = json.loads(url.text)
            ...
            if clima:
                ...
                log_entry = {
                    "Data": retry_date,
                    "Status": "Sucesso (Retry)",
                }
                log_data.append(log_entry)
            else:
                log_entry = {
                    "Data": retry_date,
                    "Status": "Erro: Sem dados (Retry)",
                }
                log_data.append(log_entry)
        except requests.exceptions.RequestException as e:
            ...
            if "429" in str(e):
                ...
                log_entry = {
                    "Data": retry_date,
                    "Status": f"Erro 429: Muitas solicitações (Retry)",
                }
                error_log.append(log_entry)
                time.sleep(60)
            else:
